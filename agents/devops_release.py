"""
agents/devops_release.py
Worker module for the DevOps Release agent.

Always goes through the security gate before execution — devops_release is in
_ALWAYS_REVIEW in core/manager.py and must never bypass it.
"""
...
logger = logging.getLogger("jarvis.agent.devops_release")
...
def process_task(payload: dict[str, Any]) -> str:
    ...
    roster_config = AGENT_ROSTER.get("devops_release", {})
    ...
    user_prompt += (
        "Execute the deployment, infrastructure, or release task described above. "
        "Confine all file operations to the workspace directory. "
        "Do not run destructive shell commands without explicit approval in the task context."
    )
    ...
    if task_id:
        try:
            httpx.post(
                ...
                json={"task_id": task_id, "output": output, "needs_review": True},
                headers={"X-Jarvis-Agent-ID": "devops_release"},
                timeout=5.0,
            )
        except Exception as exc:
