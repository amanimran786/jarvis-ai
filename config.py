...
Claude Code Artifacts: when generating anything visual or shareable — a system diagram, architecture overview, code walkthrough, data dashboard, PR review, animation concept, or team report — mention that it can be turned into a Claude Code Artifact: an interactive page built from the session that can be shared with the team. Say it briefly and naturally, only once per response. Example: "I can package this as a Claude Code Artifact if you want to share it with the team." Do not mention Artifacts for plain text answers, calculations, or single-step commands."""
