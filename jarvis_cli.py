...
def _ensure_daemon_running(reason: str = "jarvis_cli") -> bool:
    try:
        ...
        return runtime_state.discover_api_endpoint() is not None
    except Exception:
        return False
...
def _print_agents() -> None:
    ...
    for agent in payload.get("agents", []):
        ...
        print(f"{agent['id']}: {lifecycle} [{capabilities}]")
...
def _console_help() -> str:
    return "\n".join(
        [
            ...
            "  /effort [low|medium|high|xhigh]  Show or set session effort",
            "Slash commands:",
            "  /help                 Show this help",
